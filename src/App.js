...
export default class App extends React.Component {
  constructor(props){
      ...
      this.powerDrumPad = this.powerDrumPad.bind(this);
  ...
  changeBank(){
    if(this.state.power){
      this.state.currentBank === bankOne ?
        this.setState({
          currentBank: bankTwo,
          display: bankTwo[1].id
        })
      :
        this.setState({
          currentBank: bankOne,
          display: bankOne[1].id
        })
    }
  }
  powerDrumPad(){
  ...
  render() {
    return (
      <div className="pad">
        <div className="row pad__row">
          <Drumpad 
            power = {this.state.power}
            currentBank = {this.state.currentBank}
            soundName = {this.displaySoundName}
          />
          <Tools 
            power = {this.state.power}
            powerDrumPad = {this.powerDrumPad}
